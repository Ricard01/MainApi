...
public record GetExistenciayCostoQuery(int IdProducto, int IdAlmacen) : IRequest<ExistenciaCostoDto>;
...
public class GetExistenciayCostoQueryHandler : IRequestHandler<GetExistenciayCostoQuery, ExistenciaCostoDto>
{
    ...
    public async Task<ExistenciaCostoDto> Handle(GetExistenciayCostoQuery request, CancellationToken cancellationToken)
    {
        ...
        string filtroAlmacen = request.IdAlmacen > 0
            ? "AND cIdAlmacen = @IdAlmacen"
            : "";
        ...
        // 1. ReadFirstAsync porque  SUM ISNULL siempre devuelve 1 registro
        var existencia = await multi.ReadFirstAsync<double>();

        // 2. ReadFirstOrDefaultAsync porque podría no haber histórico y por eso inicializamos en ceros si no hay
        ...
        dtoResult.Existencia =
            existencia; // existencia siempre devuelve un valor valido por el SUM por eso no se incluye arriba
